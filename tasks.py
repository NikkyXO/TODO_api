...
def unassign_task(unassigned_from, task_name):
    try:
        ...
        tasks_assigned_before = db.query(AssignedTasks).filter(AssignedTasks.user_id ==
                                                               assignee.id,
                                                               AssignedTasks.task_id == task.id).all()
        ...
        for task in tasks_assigned_before:
            db.query(AssignedTasks).filter(
                AssignedTasks.user_id == assignee.id) \
                .update({"user_id": None})
